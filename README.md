...
- 実務文章では、各課題の入力前に次の文章を確認するモーダルを表示する。文章を読むための一定の待機時間を設け、待機中は開始ボタンを無効にする。ただし、秒数・時計・進捗バーは表示しない。
- 正しいキーを入力すると、入力済みのローマ字は穏やかに色が定着し、変換された日本語はインクがにじんで落ち着くようにゆっくり表示する。

### FR-04 入力位置

- 実務文章では、入力位置を確認するローマ字を下部に読みやすい大きさで表示する。
- 入力中は、正しく入力できた範囲をすべてひらがなへ変換して順に表示する。
- ローマ字の直下には、入力対象となる元の日本語文を常に全文表示する。
- Javaでは、上部に表示したコード全文の中で入力位置を示し、同じコードを下部へ重複表示しない。
- 入力に合わせて現れるひらがなと、常時表示する元の日本語文を区別して配置する。
- 「次に入力」専用カード、通常時のステータス、残り文字数、文章内進捗率は表示しない。
- 入力に合わせて現在位置を即時更新する。
- 実務文章では、入力できたひらがなを一文字ずつ穏やかに表示する。
...
- 実務文章では各文章の前に秒数表示のない確認モーダルが表示され、Javaでは表示されない。
- 実務文章の入力中は、ローマ字の直下に元の日本語文が常に表示される。
