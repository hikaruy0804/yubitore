...
トップ画面には「一文字ずつ、ゆっくり」を主見出しとして表示し、呼吸をイメージしたキーとドットのアニメーションを添える。アニメーションは設定とOSの視差効果を減らす指定に従う。

実務文章の開始ボタンは確認モーダルを開き、最初の課題の `context` と `description` を表示する。「確認したので始める」で状態を初期化して `practice` へ進む。Javaカテゴリでは確認モーダルを省略し、直接 `practice` へ進む。
...
- 初期表示が練習メニューであること
- 全9カテゴリを選択できること
- 実務文章では開始前確認モーダルが表示され、Javaでは直接練習が始まること
