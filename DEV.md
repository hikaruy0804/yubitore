...
本書は「ゆびトレ」の実装者向け機能仕様を定義する。プロダクト要求は [README.md](./README.md) を参照すること。

記載内容は現在の実装を基準とする。主要なアプリケーションロジックは [app/page.tsx](./app/page.tsx)、表示仕様は [app/globals.css](./app/globals.css) にある。
...
| ファイル | 役割 |
| --- | --- |
| `app/page.tsx` | コンテンツ、画面遷移、入力判定、指マッピング、結果集計、設定・履歴UI |
...
設定と履歴は、通常画面の上に表示するモーダルであり、`Screen`は変更しない。一時停止モーダルも `practice` の内部状態として扱う。
