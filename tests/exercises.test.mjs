...
test("practical preview waits without showing a countdown and keeps the original Japanese visible", async () => {
  const page = await readFile(new URL("../app/page.tsx", import.meta.url), "utf8");
  const css = await readFile(new URL("../app/globals.css", import.meta.url), "utf8");

  assert.match(page, /PREVIEW_READING_DELAY_MS = 10_000/);
  assert.match(page, /disabled=\{!canStart\}/);
  assert.doesNotMatch(page, /remainingSeconds|あと.*秒/);
  ...
  assert.match(css, /@keyframes ink-afterglow/);
});
